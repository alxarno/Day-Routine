import { NetworkMessage, MessageType, Action, IReady, IMessage} from "./messages";
...
export interface ITCPConnConfig {
  name: string;
  tcpdebug: boolean;
...
export class TCPConn implements IConnection {
  ...
  public Send(msg: string) {
    ...
    //   } catch (l) {
    //     if (this.data.tcpdebug) {
    //       console.log(`${this.data.name}: TCP failed to encrypt message - `, l);
    //     }
    //   }
  ...
  public Connect(addr: string, port: number) {
    ...
    this.socket.connect(port, addr, () => {
      if (this.data.tcpdebug) {
        console.log(`${this.data.name}: Connected to ${addr}:${port}`);
      }
      this.socket.setNoDelay();
    });
  }
  ...
  private async sendReady() {
    const message: IReady = {
      Action: Action.Ready,
      NetworkID: this.data.settings().NetworkID,
  ...
  private async recieved(m: Buffer, rinfo: IRequestInfo) {
    const message: NetworkMessage = JSON.parse(m.toString());
    this.data.recive({message, IP: this.address});
  }
  ...
  //   const password = this.data.settings().UserPass;
  //   if (password) {
  //     try {
  ...
  //     }
  //     try {
  //       const newuserpass = await this.data.failedDecode(this.networkID);
  //       // const dehashed = Decrypt(decrypted.toString(), newuserpass);
  //       // message = JSON.parse(dehashed);
  //       this.data.successDecode(this.networkID, newuserpass);
  //     } catch (l) {
  //       if (this.debug) {
